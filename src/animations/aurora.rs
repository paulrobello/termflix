...
impl Animation for Aurora {
    ...
    fn update(&mut self, canvas: &mut Canvas, _dt: f64, time: f64) {
        ...
        for i in 0..80 {
            ...
            let twinkle = ((t * 2.0 + i as f64 * 0.7).sin() * 0.5 + 0.5) as f64;
            if twinkle > 0.3 {
        ...
        for x in 0..canvas.width {
            ...
            for layer in 0..4 {
                ...
                let n2 = self.noise.get([fx * 7.0 + offset + 10.0, t * speed * 0.3 + 5.0]);
                let curtain_y = h * (0.08 + n1 * 0.2 + n2 * 0.06);
                let curtain_height = h * (0.35 + self.noise.get([fx * 2.0, t * 0.15 + offset]) * 0.2);
