# Garden Animation Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a "garden" ASCII animation with plants that only grow when raindrops hit them, plus a drifting sun and clouds that occasionally trigger rain.

**Architecture:** Single `src/animations/garden.rs` file using `RenderMode::Ascii` and `canvas.set_char()` for all rendering. Plants are fixed to column positions; a raindrop hitting a plant's column increments its growth stage (0→5). Clouds drift and randomly flip into rain mode, spawning raindrops. No test suite exists in this project; verification is by `cargo build` + visual run.

**Tech Stack:** Rust 2024 edition, `canvas.set_char()`, `rand::RngExt`, `Vec::retain_mut`

---

### Task 1: Create `src/animations/garden.rs`

**Files:**
- Create: `src/animations/garden.rs`

**Step 1: Create the file with all structs, constants, and `new()`**
...
**Step 2: Build to check structs and constants compile**

```bash
cargo build 2>&1 | head -30
```

Expected: errors only about missing `impl Animation` — structs should be clean.

**Step 3: Add `impl Animation for Garden`**

Append to `src/animations/garden.rs`:
...
```bash
git add src/animations/garden.rs
git commit -m "feat: add garden animation file"
```

---

### Task 2: Register `garden` in `src/animations/mod.rs`

**Files:**
- Modify: `src/animations/mod.rs`

**Step 1: Add module declaration** — insert after the last `pub mod` line (currently `pub mod vortex` or `pub mod wave`):

```rust
pub mod garden;
```

**Step 2: Add to `ANIMATIONS` const** — insert an entry in the `ANIMATIONS` slice:

```rust
("garden", "Garden scene with plants that grow when it rains"),
```

**Step 3: Add to `ANIMATION_NAMES` const** — insert `"garden"` in the slice.

**Step 4: Add to `create()` match** — insert before the `_ => panic!` arm:
...
Expected: clean build, 0 errors, 0 warnings about unused.
...
Expected: terminal shows a dark sky, `(*)` sun drifting left, `(--)` clouds, `=` ground row, `.` seeds at plant positions. Within ~10–20 seconds you should see clouds turn to `(~~)` and `|` raindrops fall; when they hit, plants advance through stages up to full colored bloom.

**Step 7: Commit**

```bash
git add src/animations/mod.rs
git commit -m "feat: register garden animation in mod.rs"
```
...
Expected: all format/lint/typecheck checks pass.

**Step 2: Fix any clippy warnings** in `src/animations/garden.rs` if present (common: `needless_pass_by_ref`, unused variables). Rerun until clean.

**Step 3: Final commit if any fixes were needed**

```bash
git add src/animations/garden.rs
git commit -m "fix: clippy warnings in garden animation"
```
