...
- [x] **[render] Smooth Brightness Transitions (small)** — Track previous-frame pixel values and blend toward target brightness over N frames. Eliminates the "flickering pixel" artifact in animations like `fire` and `plasma` where noise causes rapid on/off toggling at the threshold boundary.
